...
with sync_playwright() as playwright:
    ...
    for url in urls:
        ...
        print(f'Artykuł z tagami html: {orginal_article}')
        # Artykuł bez tagów html
        text_article = page.inner_text('article')
        print(f'Artykuł bez tagów html: {text_article}')
        # URL
        print(f'URL: {url}')
        # Data publikacji
        date_article = page.inner_text('time') if page.locator('time').count() > 0 else page.inner_text('article div p')
        date_article = parse_date(date_article)
        # Parsowanie daty
        print(f'Data publikacji: {date_article}')
    browser.close()
